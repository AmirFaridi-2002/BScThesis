...
\begin{center}
{\Large Section 4: Incorrectness Logic}\\[0.5em]
\end{center}
\setcounter{section}{2}

\section{Incorrectness Logic (IL)}\label{sec:IL}
...
\\[0.8em]
\den{C_1;C_2}_{\ok}
&\triangleq \den{C_1}_{\ok} \mathbin{;} \den{C_2}_{\ok},
&
\den{C_1;C_2}_{\er}
...
\paragraph{IL triples (under-approximate).}
...
\paragraph{Reachability characterization.}
...
\paragraph{Semantic validity of IL triples.}
...
\paragraph{A characterization lemma.}
We will use the following equivalent, ``witness-style'' formulation of under-approximation.

\begin{lemma}[Characterization of under-approximation]\label{lem:char-under}
...
\paragraph{Algebraic facts about \(\post\).}
The proofs below repeatedly use two elementary properties.

\begin{lemma}[\(\post\) distributes over union and composes over relational composition]
...
\paragraph{Two semantic invariance lemmas.}
...
\begin{lemma}[Derivable exact post-image]\label{lem:IL-exactpost}
