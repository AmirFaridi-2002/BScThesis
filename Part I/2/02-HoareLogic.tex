...
\section{Hoare Logic}\label{sec:HL}
...
the while language we defined in Section 1, several
representative examples illustrating backward reasoning, and a soundness and completeness proof
with respect to the semantics fixed in Section 1. In doing
...
\begin{definition}[]
Assertions are predicates, i.e., sets of states $P,Q \subseteq \States$. We write $\sigma \models P$ for
$\sigma \in P$.
...
\textit{Total correctness (not the focus here).}
...
\subsection{Examples (backward reasoning)}\label{sec:HL-examples}
...
\textit{Stage 1: initialization establishes $I$.}
...
\textit{Stage 2: body preserves $I$.}
...
\textit{Stage 3: conclude postcondition at loop exit.}
By \textsc{(While)}:
\[
...
operational/relational semantics of Section 1.
...
\subsection{Completeness of Hoare Logic}\label{sec:HL-completeness}
...
\paragraph{Expressiveness assumptions on the assertion logic}\label{sec:HL-assertion-logic}
...
By the equivalence from Section 1,
\[
\models \{P\}\,c\,\{Q\}
\quad\Longleftrightarrow\quad
P \subseteq \wlp(c,Q).
...
\item \textbf{$c=\texttt{if }b\texttt{ then }c_1\texttt{ else }c_2$.}
...
\item \textbf{$c=\texttt{while }b\texttt{ do }d$.}
Let
\[
...
\paragraph{Completeness via strongest postconditions.}
Using the post-image transformer $\post(c)(P)$, partial correctness validity is
