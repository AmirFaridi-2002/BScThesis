...
\smallskip
\noindent
We tacitly fix a tensor-factor ordering/parenthesization convention so that expressions such as
...
\[
  \Dsub{H}
  \;:=\;
  \{\rho \in L(H)\;:\; \rho \succeq 0 \ \wedge\ \Tr(\rho)\le 1\}.
\]
...
\paragraph{Local operators and cylindrical extension.}
...
$\rho_s$ ranges over density operators on $H_s$ (i.e.\ $\rho_s\succeq 0$, $\Tr(\rho_s)=1$).
...
\[
\infer[(\textsc{If})]
{\Conf{\mathsf{if}\ (\square m.\ M_s=m\rightarrow C_m)\ \mathsf{fi}}{\rho}\ \step{\ok}\ \Conf{C_m}{M_m^{(s)}\,\rho\,\bigl(M_m^{(s)}\bigr)^\dagger}}
{m\in \mathsf{Out}(M_s)}
\]
\noindent
