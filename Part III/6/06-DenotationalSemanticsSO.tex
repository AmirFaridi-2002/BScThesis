...
\[
  \Dsub{H} \;:=\; \{\rho\in L(H):\ \rho\succeq 0\ \wedge\ \Tr(\rho)\le 1\}.
\]
...
$\mathcal{E}(\rho)=\sum_i E_i \rho E_i^\dagger$ with $\sum_i E_i^\dagger E_i \preceq \Id$.)
...
A central design principle is \emph{locality}:
...
Recall the Löwner order $\sq$ on Hermitian operators: $A\sq B$ iff $B-A\succeq 0$.
It extends pointwise to super-operators by
\[
  \mathcal{E}\sq \mathcal{F}
  \quad\!\!\iff\quad
  \forall\rho\succeq 0,\ \mathcal{E}(\rho)\sq \mathcal{F}(\rho).
\]
...
operators describing the physical action on the subsystem. Linearity is essential: it encodes the
...
\begin{definition}[]
For super-operators $E,F:\LL(H)\to \LL(H)$, define
\[
E \sq F
\quad\Longleftrightarrow\quad
\forall\, A\succeq 0,\;\; E(A)\sq F(A).
\]
\end{definition}
...
\medskip\noindent
\textit{Case 1: $C=\error$.}
By definition, $\den{\error}(\rho)=0$ for all $\rho$. The zero map is CP (it preserves positivity
...
\smallskip\noindent
\emph{TNI.} Let $\rho\succeq 0$. Using IH (each $\den{C_m}$ is TNI),
\[
...
\[
E_n\sq E_{n+1}\quad:\!\!\iff\quad \forall \rho\succeq 0,\ E_n(\rho)\sq E_{n+1}(\rho).
\]


\begin{lemma}[Approximants are nondecreasing]\label{lem:approx-nondecreasing}
...
\end{align}
Define two auxiliary (positive) maps on inputs $\rho\succeq 0$:
\[
...
If $\rho\succeq 0$, then $T(\rho)\succeq 0$ because $\rho\mapsto M_0^{(s)}\rho(M_0^{(s)})^\dagger$
is a (single-Kraus) CP map. Also $B(\rho)\succeq 0$ because $\rho\mapsto M_1^{(s)}\rho(M_1^{(s)})^\dagger$
...
\smallskip\noindent
\underline{Base $n=0$.}
$E_0=\den{W^{(0)}}=\den{\error}$, hence $E_0(\rho)=0$ for all $\rho$.
Since $E_1(\rho)\succeq 0$ for all $\rho\succeq 0$ (by \eqref{eq:Enplus1-TB} with $n=0$),
we have $E_0(\rho)=0\sq E_1(\rho)$.

\smallskip\noindent
\underline{Step.}
Assume $E_n\sq E_{n+1}$, i.e.\ for all $X\succeq 0$, $E_n(X)\sq E_{n+1}(X)$.
We must show $E_{n+1}\sq E_{n+2}$.

Fix any $\rho\succeq 0$. Then $B(\rho)\succeq 0$, so applying the induction hypothesis to
$X=B(\rho)$ yields
\[
...
\begin{lemma}[]\label{lem:limit-QO}
Let $(E_n)_{n\ge 0}$ be a nondecreasing chain in $\QO(H)$, i.e.
\[
\forall n,\qquad E_n \sq E_{n+1}
\quad\!\!\iff\quad
\forall \rho\succeq 0,\ E_n(\rho)\sq E_{n+1}(\rho).
\]
Then the pointwise supremum
\[
E \;:=\; \bigsqcup_{n\ge 0} E_n
\]
exists, is the least upper bound of the chain, and satisfies $E\in\QO(H)$ (hence $E$ is CP and TNI).
\end{lemma}

\begin{proof}
Because $H$ is finite-dimensional, we can use the Choi--Jamio\l kowski representation.

\smallskip\noindent
\emph{-Choi matrices and their order.}
...
\]
We use the following standard facts (Choi's theorem and its trace characterization):
\begin{itemize}[leftmargin=*]
\item[] $\mathcal{E}$ is CP iff $J(\mathcal{E})\succeq 0$.
\item[] $\mathcal{E}$ is TNI iff $\Tr_1\!\bigl(J(\mathcal{E})\bigr)\preceq \Id$,
where $\Tr_1$ denotes the partial trace over the \emph{first} tensor factor of $H\otimes H$.
\item[] The assignment $\mathcal{E}\mapsto J(\mathcal{E})$ is linear and injective, hence determines
$\mathcal{E}$ uniquely from $J(\mathcal{E})$.
\end{itemize}

Write $J_n:=J(E_n)$. Since each $E_n$ is CP, we have $J_n\succeq 0$ for all $n$.
Moreover, $E_n\sq E_{n+1}$ implies $J_n\sq J_{n+1}$ because
\[
J_{n+1}-J_n
=
\bigl((E_{n+1}-E_n)\otimes \Id\bigr)\bigl(\ket{\Omega}\!\bra{\Omega}\bigr)\succeq 0
\]
(the map $E_{n+1}-E_n$ is positive by definition of $\sq$, hence so is its tensor extension).
Thus $(J_n)$ is an increasing sequence in the Löwner order.

\smallskip\noindent
\emph{-boundedness and existence of the limit.}
Each $E_n$ is TNI, so $\Tr_1(J_n)\preceq \Id$.
Taking trace and using $\Tr(J_n)=\Tr(\Tr_1(J_n))$, we obtain a uniform trace bound:
\[
\Tr(J_n)\;=\;\Tr(\Tr_1(J_n))\;\le\;\Tr(\Id)=d
\qquad\text{for all }n.
\]
Hence $(J_n)$ is an increasing sequence of PSD operators with uniformly bounded trace.
In finite dimension this implies convergence: there exists a PSD operator $J$ such that
\[
J \;=\; \lim_{n\to\infty} J_n
\qquad\text{and}\qquad
J_n \sq J \ \text{ for all }n.
\]
(One way to see this is that the eigenvalues of $J_n$ increase monotonically but their sum is bounded by $d$,
so each eigenvalue converges; equivalently, the cone of PSD matrices is closed and increasing bounded
sequences converge.)

\smallskip\noindent
\emph{-define $E$ by the limit Choi matrix.}
By injectivity of the Choi representation, there is a unique linear map
$E:L(H)\to L(H)$ such that
\[
J(E)=J.
\]
We claim $E$ is exactly the least upper bound $\bigsqcup_n E_n$.

\smallskip\noindent
\emph{-$E$ is CP and TNI.}
Since $J\succeq 0$, Choi's theorem gives that $E$ is CP.
Also, partial trace is continuous, so
\[
\Tr_1(J) \;=\; \Tr_1\!\Bigl(\lim_{n\to\infty}J_n\Bigr)
\;=\; \lim_{n\to\infty}\Tr_1(J_n)
\;\preceq\; \Id,
\]
because each $\Tr_1(J_n)\preceq \Id$.
Thus $E$ is TNI. Therefore $E\in\QO(H)$.

\smallskip\noindent
\emph{Step 5: $E$ is the least upper bound of $(E_n)$.}
First, $E$ is an upper bound.
Indeed, $J_n\sq J$ implies $J(E-E_n)=J-J_n\succeq 0$, hence $E-E_n$ is CP and therefore positive.
Thus for all $\rho\succeq 0$,
\[
E_n(\rho)\sq E(\rho),
\]
i.e.\ $E_n\sq E$.

Second, $E$ is the \emph{least} upper bound.
Let $F$ be any upper bound, so $E_n\sq F$ for all $n$.
Then $J_n=J(E_n)\sq J(F)$ for all $n$, hence taking $n\to\infty$ yields
$J=\lim_n J_n \sq J(F)$.
Therefore $J(F-E)=J(F)-J(E)\succeq 0$, so $F-E$ is CP and $E\sq F$.
This shows $E$ is the least upper bound.

\smallskip\noindent
Hence the pointwise supremum $E=\bigsqcup_{n\ge 0}E_n$ exists and belongs to $\QO(H)$.
\end{proof}
...
\smallskip\noindent
\emph{TNI.}
Let $\rho\succeq 0$. Using that $X$ is TNI and $\den{C_{\mathrm{body}}}$ is TNI
...
\begin{proof}
By denotation of $\error$, $E_0=\den{\error}=\bot$.
...
\]
Since this holds for all $\rho\succeq 0$, we conclude $\Phi(E)=\bigsqcup_{n\ge 0}\Phi(E_n)$.
